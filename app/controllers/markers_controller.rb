class MarkersController < ApplicationController
  ...
  def test
    ...
    puts " #{@result}  lat: #{params[:lat]} lon: #{params[:lon]}"
    @result[:text] = "Hi JBSers, I am working at position: (#{params[:lat]},#{params[:lon]})"
    @marker = Marker.new(lat: params[:lat], lon: params[:lon], tour:params[:tour], title: params[:title],descr: params[:descr])
    @marker.save
    @allmarkers = Marker.all
    respond_to do |format|
      format.json { render json: @allmarkers }
    end
  end
